...
public class OrdersBaseRepository implements BaseRepository<Order> {
    ...
    @Override
    public List<Order> fetchAllDataWithoutPagination(
        ...
        for (var relationship : relationships) {
            switch (relationship) {
                case "orderDetails":
                    if (!isFirstQuery) {
                        query.append(
                                " LEFT JOIN FETCH o.orderDetails od LEFT JOIN FETCH od.productQuantity opq LEFT JOIN FETCH opq.size");
                    ...
                    var tmp = pqBaseRepository.fetchAllDataWithoutPagination(
                            List.of(new WhereElements("id", pqIds, WhereClauseType.IN)), null);

                    // set product quantities to order details
                    for (var order : result)
                        for (var od : order.getOrderDetails())
                            od.setProductQuantity(tmp.stream()
                                    .filter(pq -> pq.getId()
                                            .equals(od.getProductQuantity().getId()))
                                    .findFirst()
                                    .orElse(null));
            }
        }
    ...
    @Override
    public Page<Order> fetchAllDataWithPagination(
    ...
    @Override
    public List<Order> fetchAllDataWithFirstQuery(
        ...
        String sortClause = (sort == null && pageable != null)
                ? CommonUtils.getSortClause(pageable.getSort(), "o")
