...
public class ProductsBaseRepository implements BaseRepository<Product> {
    ...
    @Override
    public List<Product> fetchAllDataWithoutPagination(
        ...
        relationships = CommonUtils.isEmptyOrNullList(relationships) ? new String[] {"productDetails"} : relationships;
            ...
            switch (relationship) {
                case "productDetails":
                    ...
                    var tmp = pdBaseRepository.fetchAllDataWithoutPagination(
                            List.of(new WhereElements("product", result, WhereClauseType.IN)), null);

                    // set product details for each product
                    result = result.stream()
                            .peek(p -> p.setProductDetails(tmp.stream()
                                    .filter(pq -> pq.getProduct().getId().equals(p.getId()))
                                    .toList()))
                            .toList();
            }
        }
    ...
    @Override
    public Page<Product> fetchAllDataWithPagination(
        ...
        for (var relationship : relationships) {
            switch (relationship) {
                case "productDetails":
                    ...
                    // fetch all relationships for each product detail
                    var tmp = pdBaseRepository.fetchAllDataWithoutPagination(
                            List.of(new WhereElements("product", content, WhereClauseType.IN)), null);
    ...
    @Override
    public List<Product> fetchAllDataWithFirstQuery(
        ...
        String sortClause = (sort == null && pageable != null)
                ? CommonUtils.getSortClause(pageable.getSort(), "p")
