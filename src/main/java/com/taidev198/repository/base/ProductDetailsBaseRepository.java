...
public class ProductDetailsBaseRepository implements BaseRepository<ProductDetail> {
    ...
    @Override
    public List<ProductDetail> fetchAllDataWithoutPagination(
        ...
        relationships =
                CommonUtils.isEmptyOrNullList(relationships) ? new String[] {"quantities", "images"} : relationships;
        boolean isFirstQuery = false;
        for (var relationship : relationships) {
            switch (relationship) {
                case "quantities":
                    if (!isFirstQuery) {
                        query.append(" LEFT JOIN FETCH pd.quantities pq LEFT JOIN FETCH pq.size");
                        ...
                        result = result.stream()
                                .peek(pd -> pd.setQuantities(tmp.stream()
                                        .filter(pq ->
                                                pq.getProductDetail().getId().equals(pd.getId()))
                                        .toList()))
                                .toList();
                    }
                    break;
                case "images":
                    if (!isFirstQuery) {
                        query.append(" LEFT JOIN FETCH pd.images");
                        result = fetchAllDataWithFirstQuery(whereElements, query.toString(), sort, null);
                        isFirstQuery = true;
                    } else {
                        ...
                        result = result.stream()
                                .peek(pd -> pd.setImages(tmp.stream()
                                        .filter(i ->
                                                i.getProductDetail().getId().equals(pd.getId()))
                                        .toList()))
                                .toList();
                    }
            }
    ...
    @Override
    public Page<ProductDetail> fetchAllDataWithPagination(
    ...
    @Override
    public List<ProductDetail> fetchAllDataWithFirstQuery(
        ...
        String sortClause = (sort == null && pageable != null)
                ? CommonUtils.getSortClause(pageable.getSort(), "pd")
