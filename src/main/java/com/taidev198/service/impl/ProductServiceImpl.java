...
public class ProductServiceImpl implements ProductService {
    ...
    public List<ProductDetailColors> getProductDetailColors(Integer id) {
        ...
        if (productDetailOptional.isPresent()) {
            ...
            List<ProductDetail> relatedProductDetails =
                    productDetailRepository
                            .findByProductId(productDetail.getProduct().getId())
                            .stream()
    ...
    public void updateProductImages(
            Integer productDetailId, List<MultipartFile> imagesToAdd, List<Integer> imagesToRemove) {
        Optional<ProductDetail> productDetailOptional =
                productDetailCustomRepository.findByIdWithRelationship(productDetailId);
    ...
    public ProductDetail updateProductInfo(ProductDetailEdit productDetailEdit) {
        Optional<ProductDetail> optionalProductDetail =
                productDetailCustomRepository.findByIdWithRelationship(productDetailEdit.getId());
        ...
        List<ProductQuantity> existingProductQuantities =
                productQuantitiesRepository.findAllByProductDetail(productDetail);
        ...
        for (ProductQuantity existingQuantity : existingProductQuantities) {
            ProductQuantity matchingQuantity = editProductQuantities.stream()
                    .filter(pq -> pq.getSize()
                            .getId()
                            .equals(existingQuantity.getSize().getId()))
                    .findFirst()
                    .orElse(null);
        ...
        for (ProductExcel productExcel : productExcels) {
            ...
            if (existingProductDetail != null) {
                ...
                List<Integer> quantityList =
                        Arrays.stream(quantities).map(Integer::parseInt).collect(Collectors.toList());
    ...
    private void updateProductQuantities(ProductDetail existingProductDetail, List<Integer> quantityList) {
        ...
        if (quantityList.size() != existingQuantities.size()) {
            throw new IllegalArgumentException(
                    "Kích thước danh sách quantityList không khớp với số lượng kích thước hiện có.");
        }
    ...
    private void addNewProductDetail(ProductExcel productExcel) {
        ...
        if (existingProduct == null) {
            existingProduct = Product.builder()
                    ...
                    .category(categoryConstants.stream()
                            .filter(c -> c.getValue().equalsIgnoreCase(productExcel.getCategory()))
                            .findFirst()
                            .orElse(null))
                    .material(materialConstants.stream()
                            .filter(c -> c.getValue().equalsIgnoreCase(productExcel.getMaterial()))
                            .findFirst()
                            .orElse(null))
                    .build();
